		...
		<div class="box-body">
			<div class="form-group">
				<label for="exampleInputEmail1">제목</label> <input type="text"
					...
					value="${riskDTO.risk_title}">
			</div>
			<div class="form-group">
				<label for="exampleInputPassword1">내용</label>
				<textarea class="form-control" name="risk_content" rows="10">${riskDTO.risk_content}</textarea>
			</div>
			<div class="form-group">
				<label for="exampleInputEmail1">프로젝트 번호</label> <input type="text"
					name="pro_num" class="form-control" value="${riskDTO.pro_num}"
					readonly="readonly">
			</div>
			<div class="form-group">
				<label for="exampleInputEmail1">작성자</label> <input type="text"
					name="mem_name" class="form-control" value="${riskDTO.mem_name}"
					readonly="readonly"> <input type="hidden" name="mem_id"
					value="${riskDTO.mem_id}"> <input type="hidden"
					name="risk_no" value="${riskDTO.risk_no}">
			</div>
			<div class="form-group">
				<label for="exampleInputEmail1">승인여부</label> <select
					id='risk_status' name='risk_status'>
					<option value=0
						<c:if test="${riskDTO.risk_status=='0'}">selected</c:if>>대기중</option>
					<option value=1
						<c:if test="${riskDTO.risk_status=='1'}">selected</c:if>>승인됨</option>
				</select>
			</div>
		</div>
